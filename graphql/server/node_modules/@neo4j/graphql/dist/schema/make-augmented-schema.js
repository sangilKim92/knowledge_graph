...
function makeAugmentedSchema(_a, { enableRegex } = {}) {
    ...
    */
        ...
        node.relationFields.forEach((rel) => {
            if (rel.union) {
                ...
                refNodes.forEach((n) => {
                    ...
                    const nodeFieldInputName = `${node.name}${graphql_compose_1.upperFirst(rel.fieldName)}${n.name}FieldInput`;
                    const nodeFieldUpdateInputName = `${node.name}${graphql_compose_1.upperFirst(rel.fieldName)}${n.name}UpdateFieldInput`;
                    const nodeFieldDeleteInputName = `${node.name}${graphql_compose_1.upperFirst(rel.fieldName)}${n.name}DeleteFieldInput`;
    ...
    const generatedResolvers = Object.assign(Object.assign({}, composer.getResolveMethods()), Object.entries(Scalars).reduce((res, [name, scalar]) => {
