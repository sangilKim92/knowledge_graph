...
function translateUpdate({ node, context }) {
    ...
    const { fieldsByTypeName } = Object.values(resolveTree.fieldsByTypeName[`Update${pluralize_1.default(node.name)}MutationResponse`]).find((field) => field.name === pluralize_1.default(camelcase_1.default(node.name)));
    if (whereInput) {
